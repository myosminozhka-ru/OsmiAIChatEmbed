...
export const CancelLeadCaptureButton = (props: LeadCaptureButtonProps) => {
  return (
    <button
      ...
      {...props}
      class={
        'chatbot-feedback-icon h-10 p-2 justify-center font-semibold focus:outline-none flex items-center disabled:opacity-50 disabled:cursor-not-allowed disabled:brightness-100 transition-all filter hover:brightness-90 active:brightness-75 ' +
        props.class
      }
  ...
  return (
    <SendButton type="submit" isDisabled={props.isDisabled || props.isLoading} class="m-0 h-14 flex items-center justify-center" {...props}>
      <Show when={!props.isLoading} fallback={<SaveLeadFallback />}>
        <span style={{ 'font-family': 'Montserrat, sans-serif' }}>Submit</span>
      </Show>
    </SendButton>
  );
};
